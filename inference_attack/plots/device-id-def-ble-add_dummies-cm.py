...
[y_test, y_pred] = deserialize("device-id-def-ble-add_dummies-cm.json")
def replace(a):
...
write_latex_table_precision_recall_f1("device-id-def-ble-add_dummies-cm.tex", y_test, y_pred)
ax.tick_params(axis='both', which='major', labelsize=CONFUSION_MATRIX_XTICKS_SIZE)
plt.tight_layout()

plt.savefig('device-id-def-ble-add_dummies-cm.png', format='png')
plt.savefig('device-id-def-ble-add_dummies-cm.png'.replace('.png', '.eps'), format='eps')
print("Written device-id-def-ble-add_dummies-cm.png/eps")
