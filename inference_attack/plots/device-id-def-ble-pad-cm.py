...
[y_test, y_pred] = deserialize("device-id-def-ble-pad-cm.json")
def replace(a):
...
write_latex_table_precision_recall_f1("device-id-def-ble-pad-cm.tex", y_test, y_pred)
plt.tight_layout()
plt.savefig('device-id-def-ble-pad-cm.png', format='png')
plt.savefig('device-id-def-ble-pad-cm.png'.replace('.png', '.eps'), format='eps')
print("Written device-id-def-ble-pad-cm.png/eps")
